...
SoftwareSerial Xbee(3, 1);

Motor motor1 = Motor(AIN1, AIN2, PWMA, offsetA, STBY);
Motor motor2 = Motor(BIN1, BIN2, PWMB, offsetB, STBY);
//Constants
const int m1p = 3; //Led to Arduino pin 3 (PWM)
const int m2p = 4; //m#p = motor pin
int m1s = 0; //m#s = motor speed
int m2s = 0;
//Variables
int value;
...
void loop() {
  ...
  if (started && ended){
   ...
   value = atoi(msg);
   digitalWrite(m1p, value);
   digitalWrite(m2p, value);
   ...
   motor1.drive(m1s, 200);
   motor2.drive(m2s, 200);
